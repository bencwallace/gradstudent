...
TEST(CtorsTest, Empty) {
  ...
  EXPECT_TRUE((t1.shape() == Array({3, 4, 5})));
  EXPECT_TRUE((t1.strides() == Array({20, 5, 1})));
}
...
TEST(CtorsTest, NonEmpty) {
    ...
    EXPECT_TRUE((t1.shape() == Array({3, 4})));
    EXPECT_TRUE((t1.strides() == Array({1, 3})));
...
TEST(CtorsTest, NonEmptyDefaultStrides) {
    ...
    EXPECT_EQ(t1.size(), 12);
    EXPECT_TRUE((t1.shape() == Array({3, 4})));
    EXPECT_TRUE((t1.strides() == Array({4, 1})));
    for (size_t i = 0; i < 12; ++i) {
        EXPECT_EQ(t1[i], i);
    }
}
...
TEST(CtorsTest, Scalar) {
    ...
    EXPECT_EQ(t1.size(), 1);
    EXPECT_TRUE((t1.shape() == Array{}));
    EXPECT_TRUE((t1.strides() == Array{}));
    EXPECT_EQ(t1[0], 3.14);
}
...
TEST(CtorsTest, Copy) {
    ...
    EXPECT_EQ(t2.size(), 4);
    EXPECT_TRUE((t2.shape() == Array{2, 2}));

    // copy should have *default* strides
    EXPECT_TRUE((t2.strides() == Array{2, 1}));
